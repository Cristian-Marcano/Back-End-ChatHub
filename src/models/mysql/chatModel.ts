...
export class ChatModel implements IChat {
    async getChats({input, id}: {input: PaginationSchema, id: UUID}): Promise<ChatMessage[]> {
        ...
        const sql = `SELECT c.id AS id, c.create_at AS create_at, IF(f.primary_user_id = UUID_TO_BIN(?), uai1.photo, uai2.photo) AS photo, IF(f.primary_user_id = UUID_TO_BIN(?), 
                    ...
                    user_account AS ua ON m.user_sending_id = ua.user_sending_id WHERE m.chat_id = c.id) 'private' AS chat_type FROM frienship AS f JOIN friendship_chat AS fc
                    ON fc.friendship_id = f.id JOIN chat AS c ON c.id = fc.chat_id WHERE f.primary_user_id = UUID_TO_BIN(?) OR f.secondary_user_id = UUID_TO_BIN(?) LIMIT ?, ?`
    ...
    async getChatById({idChat, id}: {idChat: number, id: UUID}): Promise<ChatMessage[]> {
        const sql = `SELECT c.id AS id, c.create_at AS create_at, IF(f.primary_user_id = UUID_TO_BIN(?), uai1.photo, uai2.photo) AS photo, IF(f.primary_user_id = UUID_TO_BIN(?), 
                    ...
                    (SELECT user_sending_id, chat_id, msg_text, create_at AS msg_create_at, update_at AS msg_update_at, censored, username, email, FROM message AS m JOIN
                    user_account AS ua ON m.user_sending_id = ua.user_sending_id WHERE m.chat_id = c.id) 'private' AS chat_type FROM frienship AS f JOIN friendship_chat AS fc
                    ON fc.friendship_id = f.id JOIN chat AS c ON c.id = fc.chat_id WHERE (f.primary_user_id = UUID_TO_BIN(?) OR f.secondary_user_id = UUID_TO_BIN(?)) AND id = ?`
    ...
    async getChatsByName({input, id}: {input: PaginationNameSchema, id: UUID}): Promise<ChatMessage[]> {
        ...
        const sql = `SELECT c.id AS id, c.create_at AS create_at, IF(f.primary_user_id = UUID_TO_BIN(?), uai1.photo, uai2.photo) AS photo, IF(f.primary_user_id = UUID_TO_BIN(?), 
                    ...
                    (SELECT user_sending_id, chat_id, msg_text, create_at AS msg_create_at, update_at AS msg_update_at, censored, username, email, FROM message AS m JOIN
                    user_account AS ua ON m.user_sending_id = ua.user_sending_id WHERE m.chat_id = c.id) 'private' AS chat_type FROM frienship AS f JOIN friendship_chat AS fc
                    ON fc.friendship_id = f.id JOIN chat AS c ON c.id = fc.chat_id WHERE (f.primary_user_id = UUID_TO_BIN(?) OR f.secondary_user_id = UUID_TO_BIN(?)) 
                    AND nickname = ? LIMIT ?, ?`
